...
begin
  ...
  if MessageDlg('Apagar o Registro: '+#13+#13+
                'Venda Nro: '+IntToStr(VendaId), mtConfirmation,
                            [mbYes, mbNo], 0) = mrNo
    then begin
     Result:=false;
     abort;
  ...
  try
    ...
    Try
      ...
      while not cds.Eof do begin
        if EsteItemExiste(Self.F_vendaId,
                          cds.FieldByName('produtoId').AsInteger) then
...
begin
  try
    ...
    //Pega ós códigos que estão no Cliente para Selecionar o In Not no Banco de Dados
    sCodNoCds := InNot (cds);
